...
% $Revision: 1.9.2.11 $ $Date: 2007/12/10 21:20:25 $
...
% use MODEM.PSKMOD object to Quaternary PSK (QPSK) modulate the signal, and 
...
%%
% Use INTDUMP and MODEM.PSKDEMOD object to demodulate and detect the coded
% symbols. Then sort the gray encoding array to generate the gray decoding 
% array and use the gray decoding array to decode the received symbols. Then 
% use DE2BI to convert the data to binary form. The detected symbols are plotted 
% in blue stems with circles and the original encoded symbols are plotted in 
% red stems with x's. The red stems of the transmitted signal are shadowed by
