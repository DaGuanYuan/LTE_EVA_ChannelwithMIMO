...
rng('default');
rng(167);    %seed = 67
msg_btsh = randi([0,1],1,num_bit);  %generate messege
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
pn=idinput(255,'prbs',[0 1],[0 1]);
pn=pn';
pn=[pn 0];       %PN序列长度256 m序列
tsh=[ones(1,20) pn ones(1,20)];     %Time synchronization head时间同步头 长度296
msg=[tsh msg_btsh];      %生成总初始序列
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%test, h11 = 0.5, h12 = 1, h21 = 1.5, h22 = 2
msg_ray11_01_1 = (0.5+1i)*msg_ofdm_re01_1;     %h11*x1,x1=0
msg_ray11_10_1 = (0.5+1i)*msg_ofdm_re10_1;     %h11*x1,x1=1

msg_ray12_01_2 = msg_ofdm_re01_2;     %h12*x2,x2=1
msg_ray12_10_2 = msg_ofdm_re10_2;     %h12*x2,x2=0
...
msg_ray21_01 = 1.5*msg_ofdm_re01_1;     %h21*x1,x1=0
msg_ray21_10 = 1.5*msg_ofdm_re10_1;     %h21*x1,x1=1

msg_ray22_01 = 2*msg_ofdm_re01_2;     %h22*x2,x2=1
msg_ray22_10 = 2*msg_ofdm_re10_2;     %h22*x2,x2=0
...
%% 时间同步%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
pn_qpsk=msg_qpsk(:,21:1:276);     % pn_qpsk代表经QPSK调制的本地伪随机序列
r=msg_MIMO_rx_re;     % r代表基带接收信号
%r=[r(500:600) r];     %测试
kr=length(r);
for i=1:kr
    if mod(i,length(r))==0
        gama(i)=conj(pn_qpsk(1))*r(length(r));
    else
        gama(i)=conj(pn_qpsk(1))*r(mod(i,length(r)));
    end
    for m=2:256
        if mod(m+i-1,length(r))==0
            gama(i)=gama(i)+conj(pn_qpsk(m))*r(length(r));
        else
            gama(i)=gama(i)+conj(pn_qpsk(m))*r(mod(m+i-1,length(r)));
        end
    end
end                            %计算互相关量gama
Mk=conj(gama).*gama;     %Mk为时间同步的检测统计量
Tcc=15;%
B=0;
for m=21:(256+21-1)
    B=B+conj(r(m))*r(m);
...
B=B*Tcc;    %B为门限
index=find(Mk>B) ;   %找到时间同步的检测统计量超过门限的点
start_point=index(1);      %取第一个超过门限的点为起始点
if start_point>21
    r(1:start_point-21)=[];   %删除无用点
end
msg_MIMO_rx_re=r;
%% QPSK解调%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
msg_rx(1:296)=[];          %删除时间同步头
msg_wrong = abs(msg_rx - msg_btsh);      %for turbo code
