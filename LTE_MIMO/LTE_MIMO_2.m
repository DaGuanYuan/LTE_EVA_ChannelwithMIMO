...
pilotfrequency = 1;     %导频间隔
pilot_num = fix(length(msg_qpsk)/(2*pilotfrequency));    %导频数量
rest_num2 = pilotfrequency*(pilot_num + 1) - length(msg_qpsk)/2;  %导频补零
...
for i = 1:length(msg_qpsk1)/pilotfrequency
   ...
   msg_pilot_aft2(i,:) = [0, msg_pilot1(i,:)] ;   %天线2，插入导频0
end

msg_pilot_aft_casd1 = reshape(transpose(msg_pilot_aft1), 1, size(msg_pilot_aft1,1)*size(msg_pilot_aft1,2));     %转成行
msg_pilot_aft_casd2 = reshape(transpose(msg_pilot_aft2), 1, size(msg_pilot_aft2,1)*size(msg_pilot_aft2,2));     %转成行
...
for i = 1:length(msg_qpsk1)/pilotfrequency
   ...
   msg_pilot_aft2(i,:) = [1, msg_pilot1(i,:)] ;   %天线2，插入导频0
end
...
%% 串并变换%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
group_prl = fix(length(msg_pilot_aft_casd01)/num_carrier);   %number of the groups which are divided in serial-to-parallel conversion
more_num3 = length(msg_pilot_aft_casd01) - num_carrier*group_prl; %the same as interleaving
rest_num3 = num_carrier*(group_prl + 1) - length(msg_pilot_aft_casd01);  %第二次补0个数
rest_num = 2*rest_num2 + rest_num3 ;   %总补零的个数
group_prl = group_prl + 1;

msg_pilot_aft_casd01(1, num_carrier * group_prl) = 1;   %补1是为了把不上的部分也间隔插入导频，避免后面内插NaN
msg_pilot_aft_casd10(1, num_carrier * group_prl) = 1;   %补1是为了把不上的部分也间隔插入导频，避免后面内插NaN

msg_prl01 = transpose(reshape(msg_pilot_aft_casd01, num_carrier, group_prl));     %parallel signals
msg_prl10 = transpose(reshape(msg_pilot_aft_casd10, num_carrier, group_prl));     %parallel signals

PLoc_1 = 1: (pilotfrequency+1) :length(msg_pilot_aft_casd01);   %导频位置
DLoc_1 = setxor(1:length(msg_pilot_aft_casd01), PLoc_1);      %数据位置

%% OFDM%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
N = sqrt(num_carrier);  %normalization factor
msg_idft10 = zeros(group_prl, num_carrier);
msg_idft01 = zeros(group_prl, num_carrier);
msg_ofdm10 = zeros(group_prl, num_carrier+len_cp);
msg_ofdm01 = zeros(group_prl, num_carrier+len_cp);
msg_ray0 = zeros(num_carrier+len_cp, group_prl);
msg_ray = zeros(group_prl, num_carrier+len_cp);
msg_nocp01 = zeros(group_prl, num_carrier);
msg_nocp10 = zeros(group_prl, num_carrier);
msg_nocp_ray = zeros(group_prl, num_carrier);
msg_nocp_est = zeros(group_prl, num_carrier);
msg_fft01 = zeros(group_prl, num_carrier);
msg_fft10 = zeros(group_prl, num_carrier);
MSG_PRL = zeros(group_prl, num_carrier);
MSG_NOCP_EST = zeros(group_prl, num_carrier);
H_RAY = zeros(group_prl, num_carrier);
% h_ray = zeros(group_prl, num_carrier+len_cp);
cp01 = zeros(group_prl, len_cp);  %initial
cp10 = zeros(group_prl, len_cp);  %initial

for j = 1 : group_prl
   msg_idft01(j,:) = ifft(msg_prl01(j,:))/N;    %idft
   msg_idft10(j,:) = ifft(msg_prl10(j,:))/N;    %idft

   cp01(j,:) = msg_idft01(j,num_carrier-len_cp+1:num_carrier);   %cp, 12bit
   cp10(j,:) = msg_idft10(j,num_carrier-len_cp+1:num_carrier);   %cp, 12bit
    
   msg_ofdm01(j,:) = cat(2,cp01(j,:),msg_idft01(j,:));    %add cp,concatenate matrix
   msg_ofdm10(j,:) = cat(2,cp10(j,:),msg_idft10(j,:));    %add cp,concatenate matrix
...
msg_ofdm_re10 = reshape(transpose(msg_ofdm10),size(msg_ofdm10,1)*size(msg_ofdm10,2)/numTx,numTx);   %每一列是一个天线的输入
msg_ofdm_re01 = reshape(transpose(msg_ofdm01),size(msg_ofdm01,1)*size(msg_ofdm01,2)/numTx,numTx);   %每一列是一个天线的输入
...
msg_MIMO01 = zeros(size(msg_ofdm_re01,1) ,2);
msg_MIMO10 = zeros(size(msg_ofdm_re10,1) ,2);

[msg_ray11_01, pathgains11] = step(Ray_chan11, msg_ofdm_re01(:,1));     %h11*x1,x1=0
msg_ray11_10 = step(Ray_chan11, msg_ofdm_re10(:,1));     %h11*x1,x1=1

[msg_ray12_01, pathgains12] = step(Ray_chan12, msg_ofdm_re01(:,2));     %h12*x2,x2=1
msg_ray12_10 = step(Ray_chan12, msg_ofdm_re10(:,2));     %h12*x2,x2=0

msg_MIMO01(:,1) = msg_ray11_01 + msg_ray12_01;      %y1 = h11*x1 + h12*x2,x1=0,x2=1
msg_MIMO10(:,1) = msg_ray11_10 + msg_ray12_10;      %y1 = h11*x1 + h12*x2,x1=1,x2=0

[msg_ray21_01, pathgains21] = step(Ray_chan21, msg_ofdm_re01(:, 1));     %h21*x1,x1=0
msg_ray21_10 = step(Ray_chan21, msg_ofdm_re10(:, 1));     %h21*x1,x1=1

[msg_ray22_01, pathgains22] = step(Ray_chan22, msg_ofdm_re01(:, 2));     %h22*x2,x2=1
msg_ray22_10 = step(Ray_chan22, msg_ofdm_re10(:, 2));     %h22*x2,x2=0

msg_MIMO01(:,2) = msg_ray21_01 + msg_ray22_01;      %y2 = h21*x1 + h22*x2
msg_MIMO10(:,2) = msg_ray21_10 + msg_ray22_10;      %y2 = h21*x1 + h22*x2

%     [msg_MIMO, MIMO_PATHGAINS] = step(MIMO_chan, msg_ofdm_re);   %pass MIMO, y = Hx
msg_awgn01 = awgn(msg_MIMO01,SNR_awgn,'measured',19980611);      %pass awgn,接收信号y = Hx + n
msg_awgn10 = awgn(msg_MIMO10,SNR_awgn,'measured',19980611);      %pass awgn,接收信号y = Hx + n

msg_MIMO_re01 = transpose(reshape(msg_awgn01, size(msg_awgn01,1)*size(msg_awgn01,2)/group_prl,group_prl));   %返回到并行子载波形式
msg_MIMO_re10 = transpose(reshape(msg_awgn10, size(msg_awgn10,1)*size(msg_awgn10,2)/group_prl,group_prl));   %返回到并行子载波形式

for j = 1 : group_prl       
   %% DFT%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   msg_nocp01(j,:) = msg_MIMO_re01(j,13:num_carrier+len_cp);    %wipe off cp
   msg_nocp10(j,:) = msg_MIMO_re10(j,13:num_carrier+len_cp);    %wipe off cp

   msg_fft01(j,:) = N*fft(msg_nocp01(j,:));     %idft
   msg_fft10(j,:) = N*fft(msg_nocp10(j,:));     %idft


end

%% 并串变换%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
msg_casd01 = transpose(reshape(transpose(msg_fft01),size(msg_fft01,1)*size(msg_fft01,2),1));   %按照接收mimo信号
msg_casd10 = transpose(reshape(transpose(msg_fft10),size(msg_fft10,1)*size(msg_fft10,2),1));   %按照接收mimo信号

% msg_casd01_0 = transpose(msg_casd01(:,1));  %y1, x1 = 0,x2 = 1接收，h12
% msg_casd01_1 = transpose(msg_casd01(:,2));  %y2, x1 = 0,x2 = 1接收, h22
% msg_casd10_1 = transpose(msg_casd10(:,1));  %y1, x1 = 1,x2 = 0接收，h11
% msg_casd10_0 = transpose(msg_casd10(:,2));  %y2, x1 = 1,x2 = 0接收，h21

%% 信道估计%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%LS channel estimate%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
h01 = msg_casd01(PLoc_1);     %notice that pilot is 1
h10 = msg_casd10(PLoc_1);     %notice that pilot is 1

msg_casd_len = length(msg_casd01);

h_est01 = interp1(PLoc_1, h01, 1:msg_casd_len);
h_est10 = interp1(PLoc_1, h10, 1:msg_casd_len);

h_est01(msg_casd_len - rest_num3 + 1 :msg_casd_len) = [];   %去掉串并变换补的0
h_est10(msg_casd_len - rest_num3 + 1 :msg_casd_len) = [];   %去掉串并变换补的0

PLoc = 1: (pilotfrequency+1) :length(h_est01); 
DLoc = setxor(1:length(h_est01), PLoc);      %数据位置
h_ray01 = h_est01(DLoc);
h_ray10 = h_est10(DLoc);
h_ray_len = length(h_ray01);
h_ray01(h_ray_len - 2*rest_num2 + 1 : h_ray_len) = [];  %去掉导频补的0，要分开去,不然会有余数问题
h_ray10(h_ray_len - 2*rest_num2 + 1 : h_ray_len) = [];  %去掉导频补的0，要分开去，不然会有余数问题

h_ray01_re = reshape(h_ray01,length(h_ray01)/2,2);    %转换成天线接收形式
h_ray10_re = reshape(h_ray10,length(h_ray10)/2,2);    %转换成天线接收形式
h11 = transpose(h_ray10_re(:,1));
h12 = transpose(h_ray01_re(:,1));
h21 = transpose(h_ray10_re(:,2));
h22 = transpose(h_ray01_re(:,2));

msg_casd01(msg_casd_len - rest_num3 + 1 :msg_casd_len) = [];     %去掉串并变换补的0
msg_casd01 = msg_casd01(DLoc);
msg_casd01(length(msg_casd01) - 2*rest_num2 + 1 : length(msg_casd01)) = []; %去掉导频补的0，要分开去，不然会有余数问题
msg_casd01_re = reshape(msg_casd01, length(msg_casd01)/2,2);  %转化问天线输出形式
%%  MIMO接收%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
flag = zeros(1,16);
msg_MIMO_rx = zeros(size(msg_casd01_re,1)*size(msg_casd01_re,2)/2, 2);    %ML后的接收信号
for i = 1 : size(msg_casd01_re,1)*size(msg_casd01_re,2)/2
    for j = 1:16
        y(j,1) = h11(i)*x(j,1) + h12(i)*x(j,2);
        y(j,2) = h21(i)*x(j,1) + h22(i)*x(j,2);
        flag(j) = norm(msg_casd01_re(i,:)-y(j,:));   %in order to compare 求范数=-=立个flag,y is Hx
    end
