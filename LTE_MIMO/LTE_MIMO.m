...
%         'NormalizeChannelOutputs', true);
    ...
    msg_ray11 = step(Ray_chan11, msg_ofdm_re(:,1));     %h11*x1
    msg_ray12 = step(Ray_chan12, msg_ofdm_re(:,2));     %h12*x2
    msg_MIMO(:,1) = msg_ray11 + msg_ray12;      %y1 = h11*x1 + h12*x2
    msg_ray21 = step(Ray_chan21, msg_ofdm_re(:, 1));     %h21*x1
    msg_ray22 = step(Ray_chan22, msg_ofdm_re(:, 2));     %h22*x2
...
%     [msg_MIMO, MIMO_PATHGAINS] = step(MIMO_chan, msg_ofdm_re);   %pass MIMO, y = Hx
    ...
    %% 并串变换%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    msg_casd = reshape(transpose(msg_fft),size(msg_fft,1)*size(msg_fft,2),2);   %按照接收mimo信号
    %%  MIMO接收%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for j = 1: 4
        for k = 0:3
            x(4*k+j, 1) = x_base(1) * exp(k*1i*pi/2);
            x(4*k+j, 2) = x_base(2) * exp(j*1i*pi/2);
        end        
    end
    ...
    for i = 1 : size(msg_casd,4)*size(msg_casd,2)/2
        for j = 1:16
            y(j,1) = h11(i)*x(j,1) + h12*x(j,2);
            y(j,2) = h21(i)*x(j,1) + h22*x(j,2);
    ...
    msg_MIMO_rx_re(msg_MIMO_len - rest_num2 +1 : msg_MIMO_len) = [];     %去掉补的0
